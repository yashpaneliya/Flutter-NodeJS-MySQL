...
class _AssignedTaskState extends State<AssignedTask> {
                          ...
                          slivers:<Widget> [
                            ...
                            SliverList(
                                delegate: SliverChildBuilderDelegate((BuildContext context, int index) {
                                  return (index<snapshot.data.length && snapshot.data.elementAt(index).status=='0')?
                                  Container(
                                            ...
                                            itemBuilder: (context){
                                              return ["Complete","Delete"].map((e) {
                                                return PopupMenuItem(
                                                  child: Text(e),
                                                  value: [widget.id,snapshot.data.elementAt(index).title,snapshot.data.elementAt(index).tid,snapshot.data.elementAt(index).date],
                                                );
                                              } ).toList();
                                            }
                                          ),
                            ...
                            SliverToBoxAdapter( 
                              child: Container(
                                  ...
                                  padding: EdgeInsets.only(top:4.0,bottom: 4.0,left: 7.0) ,
                            ...
                            SliverList(                              
                                delegate: SliverChildBuilderDelegate((BuildContext context, int index) {
                                  return (index<snapshot.data.length && snapshot.data.elementAt(index).status=='1')?
                                  Container(
                                    margin: EdgeInsets.only(left:10.0,right: 10.0,top: 10.0),
                                    width: MediaQuery.of(context).size.width-10.0,
                                    child: Card(
  ...
  void performAction(List task)async{
    if(await completeAssignedByMeTask(task.elementAt(0), task.elementAt(1),task.elementAt(2), task.elementAt(3))){
      setState(() {
          ...
          return Container(
            ...
            child: ListView(
              ...
              Row(
                ...
                children: [
                Text('Works',style: TextStyle(fontWeight:FontWeight.bold,fontSize: 25.0,color: Colors.black),),
                IconButton(icon: Icon(Icons.settings_ethernet), onPressed: (){})
              ],),
              RichText(
                text:TextSpan(
                  children: [
                  TextSpan(text: 'The tasks assigned ',style: TextStyle(color: Colors.grey)),
                  TextSpan(text: 'to others',style: TextStyle(color: Colors.green)),
